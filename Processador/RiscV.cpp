	...
	while(true) {
        ...
        switch (opcode) {
            ...
            case JAL:
                ...
                break;
            case LW:
                if((int16_t)(breg[rs1]+Imm) < 0x8000) {
                    P_out_Data.write((int16_t)(breg[rs1]+Imm)); //casting para 16 bits pois a memoria so tem 16 bits
                    Data_Write(false);
                    breg[rd] = P_in_Data.read();
                } else {
                    P_out_Inst.write((int16_t)(breg[rs1]+Imm)); //casting para 16 bits pois a memoria so tem 16 bits
                    Data_Write(false);
            ...
            case SW:
                if((int16_t)(breg[rs1]+Imm) < 0x8000) {
                    ...
                    P_out_Inst.write((int16_t)(breg[rs1]+Imm));
                    Data.write(breg[rs2]);
                    Write_Signal.write(true);
                }
                break;
